"""Module to solve part 2 of advent of code 2023-2"""
...
def main(root: str, run_type: Run = Run.TEST) -> int:
    ...
    for line in inp:
        maxes = [0, 0, 0]
        colours_sects: list[str] = line.split(":")[-1].split(";")
        for section in colours_sects:
            colours = [col.strip().split(" ") for col in section.split(",")]
            for (count, colour) in colours:
                match colour[0]:
                    case 'r':
                        if int(count) > maxes[0]:
                            maxes[0] = int(count)
                    case 'g':
                        if int(count) > maxes[1]:
                            maxes[1] = int(count)
                    case 'b':
                        if int(count) > maxes[2]:
                            maxes[2] = int(count)
                    case _:
                        raise TypeError("Unexpected string found")
