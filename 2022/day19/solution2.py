...
def main(enabled_print=True, test=False):
    if test:
        with open(r"2022\day19\test.txt", 'r') as f:
    ...
    for (b_id, info) in blueprints:
        ...
        for time in range(run_time):
            ...
            max_ore = max_ore_bots*(run_time - time)
            max_clay = max_clay_bots*(run_time - time)
            max_obsidian = max_obsidian_bots*(run_time - time)
