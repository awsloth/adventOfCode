"""Module to solve part 1 of advent of code 2019-13"""
...
def main(root: str, run_type: Run = Run.TEST) -> int:
    ...
    while not comp.halted:
        x = comp.get_next_output(1)
        y = comp.get_next_output(2)
        tile_id = comp.get_next_output(3)
    ...
    total_block = sum(len([x for x in y.values() if x == TileMap.BLOCK]) for y in screen.values())
