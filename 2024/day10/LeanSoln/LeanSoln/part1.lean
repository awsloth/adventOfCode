...
def List.unique : List (Nat × Nat) → List (Nat × Nat)
  | [] => []
  | (x :: xs) => if xs.contains x then xs.unique else x :: xs.unique
...
def answer (grid : List (List Nat)) : Nat := List.sum (List.map (λ start ↦ ((validWalks 9 grid start).unique).length) (locateHeads grid))
