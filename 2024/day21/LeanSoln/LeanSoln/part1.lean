...
def calcAllSteps (nums : List NumPad) : List Step := stepsForNRobot $ prependAStep $
                                                     stepsForNRobot $ prependAStep $
                                                     stepsForCode nums


def answer : Nat := Nat.sum $ List.zipWith (.*.) (List.map (λ s ↦ s.dropLast.asString.toNat!) $ inputs data)
                      ((List.map (List.length ∘ calcAllSteps ∘ prependA ∘ readToNumPad) $ inputs data))
