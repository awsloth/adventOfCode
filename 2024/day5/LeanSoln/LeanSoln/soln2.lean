...
def invalidRules (xs : List String) : List (List Nat) := List.filter (λ x ↦ Bool.not (validLine (intRelRules $ relRules xs) x)) (intPrintRules $ printRules xs)
...
def sortLine (xs : List Nat) (relations : List (Nat × Nat)): List Nat := (Array.qsort xs.toArray (λ x ↦ (λ y ↦ rel x y relations))).toList

#eval sum (List.map middle (List.map (sortLine . (intRelRules $ relRules lines)) (invalidRules lines)))
