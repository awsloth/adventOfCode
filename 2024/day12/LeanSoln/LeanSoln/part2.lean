...
def Range.countSame : Range → Range → Nat
  | (sp, ep), (sq, eq) => if (sp == sq) then if (ep == eq) then 2 else 1 else if (ep == eq) then 1 else 0
...
def Patch.perimeter : Patch → Nat
  | (_, lines) => List.sum $
                  List.map (λ (above, line) ↦ (line.length * 2 -
                                              (List.sum $ List.map (λ block ↦
                                                                    List.sum $ List.map (λ a ↦
                                                                    block.countSame a) above) line)
                                              )*2
                           )
