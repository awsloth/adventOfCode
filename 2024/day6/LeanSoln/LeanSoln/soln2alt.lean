...
#eval! let corners := cornersToSquares walk.tail!; (List.filter (λ obs ↦ (loopWalk obs guardStart [guardStart] 1000).fst) (List.map (newBoard obst .) (uniqueWalk corners))).length
